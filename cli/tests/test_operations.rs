...
fn test_op_diff() {
    ...
    // `jj op diff @` should behave identically to `jj op diff --from
...
fn test_op_diff_patch() {
    ...
    "###);
    let stdout = test_env.jj_cmd_success(&repo_path, &["op", "diff", "--op", "@-", "-p"]);
    insta::assert_snapshot!(&stdout, @r###"
    From operation b51416386f26: add workspace 'default'
      To operation 816a0ba42aa6: snapshot working copy
    ...
    let stdout = test_env.jj_cmd_success(&repo_path, &["op", "diff", "--op", "@", "-p"]);
    insta::assert_snapshot!(&stdout, @r###"
    From operation 816a0ba42aa6: snapshot working copy
      To operation 64bb2702ac30: new empty commit
    ...
    "###);
    let stdout = test_env.jj_cmd_success(&repo_path, &["op", "diff", "-p"]);
    insta::assert_snapshot!(&stdout, @r###"
    From operation 06fc05072dff: snapshot working copy
      To operation f40519d9c6c3: squash commits into f3220d78b25353bd8fd65988018004b962d0894a
    ...
    let stdout = test_env.jj_cmd_success(&repo_path, &["op", "diff", "-p"]);
    insta::assert_snapshot!(&stdout, @r###"
    From operation f40519d9c6c3: squash commits into f3220d78b25353bd8fd65988018004b962d0894a
      To operation 318c9022ebc5: abandon commit fec3065ddf4c348827a1e8eee6c49db252135023
